...
- [x] task-01: news_archive schema v2 与 IMAGES_DIR（覆盖：FR-09, D-006@v1）
- [x] task-02: 14A upsert 保留 14B enrichment 字段（覆盖：FR-07, D-006@v1）

依赖说明：task-02 依赖 task-01 的 schema/路径常量语义，但二者都限定在 `news_archive.py`。

## Wave 2 — archive_enrich core（新增增强模块）

- [x] task-03: 新增 archive_enrich.py CLI 与路径工具（覆盖：FR-05, FR-09, D-004@v1）
- [x] task-04: 实现正文补全状态机（覆盖：FR-01, FR-02, D-001@v1, D-002@v1）
- [x] task-05: 实现首图提取与下载（覆盖：FR-03, FR-04, FR-08, D-003@v1, D-007@v1）
- [x] task-06: 实现 JSONL enrich 读写与统计（覆盖：FR-05, FR-06, FR-09, D-004@v1, D-005@v1）

依赖说明：task-03 提供 CLI/路径骨架；task-04 和 task-05 可在同一新文件中独立实现；task-06 汇总 task-03~05。

## Wave 3 — pipeline integration（主流程接入）

- [x] task-07: step4 接入 archive_enrich best-effort（覆盖：FR-06, D-005@v1）

依赖说明：task-07 依赖 Wave 2 的 `archive_enrich.enrich_archive_best_effort` 已存在。

## Wave 4 — Tests + verification（回归与验收）

- [x] task-08: 新增 tests/test_archive_enrich.py（覆盖：FR-01~FR-06, FR-08, FR-09, D-001@v1~D-005@v1, D-007@v1）
- [x] task-09: 扩展 tests/test_news_archive.py（覆盖：FR-07, FR-09, D-006@v1）
- [x] task-10: 运行验证命令（覆盖：FR-01~FR-09）
...
- [x] `python3 tests/test_news_archive.py` 通过
- [x] `python3 tests/test_archive_enrich.py` 通过
- [x] `python3 archive_enrich.py --date <test-date> --dry-run` 不写 JSONL、不下载图片，并输出统计
- [x] 正文补全不调用 LLM；失败时不写 fake body
- [x] 非 top10 记录不下载图片，`image_status` 为 `not_selected`
- [x] top10 图片成功时同时有 `image_url` 与 `image_path`
- [x] 14A `archive_articles` 重新 upsert 不会清空既有 `body*` / `image*` 字段
- [x] `step4.py` 自动触发 enrichment 失败时不影响 `1新闻_链接.md` 写入和主流程返回
- [x] Brownfield 兼容：不修改 `run_all.sh`，不改变 `1新闻_链接.md` / `2新闻_已审核.md` / `3新闻_概述.md` / HTML / PNG 格式
- [x] 无新增第三方依赖，无 type hints，CLI 保持手写 parse_args 风格
