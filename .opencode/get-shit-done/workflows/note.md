...
- **Global scope**: `/mnt/e/Daily/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/mnt/e/Daily/.opencode/notes/`)
...
2. Glob `/mnt/e/Daily/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (/mnt/e/Daily/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `/mnt/e/Daily/.opencode/notes/` — works in any directory
