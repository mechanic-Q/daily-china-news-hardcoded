...
- **敏感词剔除（全链路）**：标题或正文含 `daily/common.py:BLOCKED_NEWS_TERMS`（习近平/习主席/习总书记）的条目整体剔除，报纸与视频均不出现。闸门设在 step4（标题级，进 LLM 前剔除省开销）、step6（标题+抓取后正文级）、step7（组装概述前最后一道，防重跑旧数据绕过）。日志省略敏感标题原文，不留存
