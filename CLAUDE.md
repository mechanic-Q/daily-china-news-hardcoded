...
| 文件 | 行数 | 职责 |
|------|------|------|
| `step1_3.py` | 475 | 7 信源异步采集器，HTTP-200 校验 |
| `step4.py` | 434 | 关键词加权评分 + LLM 分类，选前 10 |
| `step6.py` | 286 | 5 层正文提取策略链（TRS_Editor → article → class 匹配 → `<p>` 回退）|
| `step7.py` | 271 | LLM 摘要生成 + 诊断重试 |
| `step8.py` | 513 | HTML 双栏渲染 + Chromium 截图 + Pillow 裁剪 |
| `run_all.sh` | 51 | 管道编排 |

## 分支策略

见 `AGENTS.md` — 每个 GSD phase 以 `phase-{NN}-{name}` 命名分支，PR merge 回 main。
